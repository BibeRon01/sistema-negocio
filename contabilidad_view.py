...
def _legacy_reportes_dgii_desactivado():
    raise RuntimeError(
        "El generador legado fue retirado. Use únicamente la Academia DGII educativa."
    )
...
def render_cierre_periodo():
    ...
    st.caption(
        "Congela un mes del registro contable interno después de validar el "
        "balance y el inventario. No prepara declaraciones DGII."
    )
...
- Registra el cierre interno del mes y evita modificaciones posteriores.
- Genera un resumen administrativo descargable de ventas, ITBIS informativo,
  compras, gastos y margen.
- No genera ni sustituye IT-1, IR-2, 606, 607 u otro documento oficial.
    """)
    ...
    costo_ventas_cp = float(calcular_costo_ventas_real(desde_cp, hasta_cp, df_v_cp))
    margen_operativo_cp = total_ventas_cp - total_itbis_cp - costo_ventas_cp - total_gastos_cp
    ...
    r3.metric("Costo de ventas", f"RD$ {costo_ventas_cp:,.2f}")
    r4.metric("Gastos",         f"RD$ {total_gastos_cp:,.2f}")
    r5.metric("Margen operativo interno",  f"RD$ {margen_operativo_cp:,.2f}",
              delta="Positivo" if margen_operativo_cp >= 0 else "Negativo",
              delta_color="normal" if margen_operativo_cp >= 0 else "inverse")
    st.caption(f"Ventas internas: {len(df_v_cp)} | Compras: {len(df_c_cp)} | Gastos: {len(df_g_cp)}")
    ...
    resumen_txt = (
        ...
        f"COSTO DE VENTAS:    RD$ {costo_ventas_cp:>12,.2f}\n"
        f"GASTOS OPERATIVOS:  RD$ {total_gastos_cp:>12,.2f}\n"
        f"MARGEN OPERATIVO:   RD$ {margen_operativo_cp:>12,.2f}\n"
        f"{'='*44}\n"
        f"ITBIS EN VENTAS (REFERENCIA, NO DECLARACIÓN): RD$ {total_itbis_cp:>12,.2f}\n"
        f"Ventas internas:    {len(df_v_cp)}\n"
    ...
    with col_b2:
        ...
        if st.button("🔒 Ejecutar Cierre de Período", key="btn_cp_exec",
            ...
            if confirmacion.strip().upper() == f"CERRAR {MESES[mes_cierre].upper()}":
                try:
                    from api_client import cerrar_periodo

                    result = cerrar_periodo(
                        int(ano_cierre),
                        int(mes_cierre),
                        "Cierre confirmado desde la aplicación",
                    )
                    limpiar_cache_datos()
                    st.success(
                        f"✅ Período **{MESES[mes_cierre]} {ano_cierre}** cerrado "
                        "después de validar balance e inventario."
                    )
                    with st.expander("Resumen persistido del cierre"):
                        st.json(result.get("resumen") or {})
                    st.balloons()
...
# La definición educativa es la autoridad final del módulo. Sustituye el
# generador heredado situado al comienzo de este archivo.
def render_reportes_dgii():
    st.title("📘 Academia DGII — Orientación básica")
    st.warning(
        "Este módulo no llena, genera ni envía formularios 606, 607, 608, "
        "609, IT-1 o IR-2. Tampoco calcula una declaración oficial."
    )

    tema = st.selectbox(
        "Tema que desea conocer",
        [
            "Obligaciones y calendario",
            "Compras y gastos — concepto del 606",
            "Ventas — concepto del 607",
            "Comprobantes anulados — concepto del 608",
            "Pagos al exterior — concepto del 609",
            "ITBIS e IT-1",
            "ISR empresarial e IR-2",
        ],
    )

    explicaciones = {
        "Obligaciones y calendario": (
            "Las obligaciones y fechas dependen del tipo de contribuyente y de "
            "su registro. Revise su Oficina Virtual y confirme con su contador."
        ),
        "Compras y gastos — concepto del 606": (
            "El 606 organiza información de compras de bienes, servicios y gastos "
            "sustentados. La clasificación y el archivo final deben prepararse en "
            "el proceso oficial indicado por la DGII."
        ),
        "Ventas — concepto del 607": (
            "El 607 resume operaciones de venta y comprobantes emitidos. Este "
            "sistema conserva datos internos, pero no produce el archivo oficial."
        ),
        "Comprobantes anulados — concepto del 608": (
            "El 608 informa secuencias anuladas cuando corresponda. Una anulación "
            "interna del POS no sustituye el procedimiento fiscal."
        ),
        "Pagos al exterior — concepto del 609": (
            "El 609 se relaciona con pagos de servicios al exterior sujetos a las "
            "reglas aplicables. Requiere clasificación profesional."
        ),
        "ITBIS e IT-1": (
            "El IT-1 considera débitos, adelantos, retenciones, ajustes y créditos "
            "admitidos. Un total de ventas por sí solo no determina la declaración."
        ),
        "ISR empresarial e IR-2": (
            "El IR-2 parte de estados financieros, conciliación fiscal, partidas "
            "deducibles y no deducibles y otros anexos. No es simplemente aplicar "
            "un porcentaje a la utilidad mostrada en pantalla."
        ),
    }
    st.info(explicaciones[tema])

    st.markdown("### Qué debe preparar para su contador")
    st.markdown(
        """
        - Ventas y comprobantes del período.
        - Compras, gastos y documentos de soporte.
        - Notas de crédito, anulaciones y devoluciones.
        - Retenciones, anticipos y pagos realizados.
        - Estados de cuenta y conciliaciones.
        - Nómina, TSS y documentos laborales correspondientes.
        """
    )
    st.markdown("### Fuentes oficiales")
    st.link_button(
        "Biblioteca y formatos de envío — DGII",
        "https://dgii.gov.do/publicacionesOficiales/bibliotecaVirtual/Paginas/default.aspx",
    )
    st.link_button(
        "Oficina Virtual — DGII",
        "https://dgii.gov.do/servicios/Paginas/oficinaVirtual.aspx",
    )
    st.caption("Contenido educativo; verifique fecha, versión y obligación directamente con DGII.")
