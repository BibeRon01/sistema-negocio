...
NOMINA_PARAMETROS_POR_ANO = {
    2026: {
        ...
        "ISR_TRAMOS": [
            (72260.25, 6648.00, 0.25),
            (52027.42, 2601.33, 0.20),
            (34685.00, 0.0,     0.15),
        ]
    }
}
...
def obtener_parametros_nomina(ano: int = None) -> tuple[dict, list[tuple]]:
    ...
    if ano != 2026:
        raise ValueError(
            f"No hay parámetros de nómina verificados para {ano}. "
            "Cárguelos en nomina_parametros antes de calcular."
        )
    params = NOMINA_PARAMETROS_POR_ANO[ano]
...
def calcular_nomina_completa(
    ...
    periodo: str = "mensual",
    ano: int = None,
    arl_tasa: float = 0.011,
) -> dict:
    TSS, ISR_TRAMOS = obtener_parametros_nomina(ano)
    """Calcula una cuota del período a partir del salario mensual."""
    factor = {"mensual": 1.0, "quincenal": 0.5, "semanal": 12.0 / 52.0}.get(periodo)
    if factor is None:
        raise ValueError("Período de nómina inválido.")
    bruto_mes = float(sueldo_bruto_mensual)
    ...
    sfs_e_mes = round(base_sfs * TSS["sfs_empleado"], 2)
    afp_e_mes = round(base_afp * TSS["afp_empleado"], 2)
    tss_e_mes = sfs_e_mes + afp_e_mes
    base_isr = bruto_mes - tss_e_mes

    isr_mes = 0.0
    for limite, fijo, tasa in ISR_TRAMOS:
        if base_isr > limite:
            isr_mes = round(fijo + (base_isr - limite) * tasa, 2)
            break

    sueldo_periodo = round(bruto_mes * factor, 2)
    sfs_e = round(sfs_e_mes * factor, 2)
    afp_e = round(afp_e_mes * factor, 2)
    isr = round(isr_mes * factor, 2)
    tss_e = sfs_e + afp_e
    neto_periodo = round(sueldo_periodo - tss_e - isr, 2)
    neto_mes = round(bruto_mes - tss_e_mes - isr_mes, 2)

    # Costo empleador (sobre sueldo mensual topeado)
    sfs_p      = round(base_sfs * TSS["sfs_empleador"] * factor, 2)
    afp_p      = round(base_afp * TSS["afp_empleador"] * factor, 2)
    if not 0.01 <= float(arl_tasa) <= 0.016:
        raise ValueError("La tasa ARL debe estar entre 1.00% y 1.60%.")
    arl_p      = round(base_arl * float(arl_tasa) * factor, 2)
    infotep_p  = round(bruto_mes * TSS["infotep_empleador"] * factor, 2)
    tss_p      = sfs_p + afp_p + arl_p + infotep_p
    costo_total = round(sueldo_periodo + tss_p, 2)

    return {
        "sueldo_bruto":      sueldo_periodo,
...
<table>
  ...
  <tr><td>ARL (Riesgos Laborales) — {float(d.get('arl_tasa', 0))*100:.2f}%</td><td style="text-align:right">RD$ {d['arl_empleador']:,.2f}</td></tr>
...
def render_empleados():
    ...
    with st.expander("📥 Subir Excel / CSV de empleados"):
        st.write(
            "Columnas requeridas: nombre, puesto, sueldo, tipo_salario, "
            "frecuencia_pago y arl_tasa_pct (por ejemplo 1.10)."
        )
        ...
        if archivo is not None and st.button("Cargar empleados"):
            ...
            faltan = [
                c for c in [
                    "nombre", "puesto", "sueldo", "tipo_salario",
                    "frecuencia_pago", "arl_tasa_pct",
                ...
                for _, row in df.iterrows():
                    insertar(
                        "empleados",
                        {
                            ...
                            "arl_tasa": float(limpiar_numero(row["arl_tasa_pct"]) or 0) / 100.0,
    ...
    with st.expander("➕ Agregar empleado manual", expanded=True):
        ...
        with c1:
            ...
            arl_pct = st.number_input(
                ...
                step=0.1,
                key="emp_arl_pct",
                help="Confirme la tasa aplicable a la empresa antes de registrar nómina.",
            )
...
def render_nomina():
    ...
    with tab_calc:
        ...
        with c1:
            ...
            if emp_sel and "sueldo" in empleados_df.columns:
                ...
                if not row_emp.empty:
                    ...
                    arl_tasa_empleado = limpiar_numero(row_emp.iloc[0].get("arl_tasa"))

            sueldo_bruto = st.number_input("Sueldo bruto mensual (RD$)", min_value=0.0,
        ...
        with c2:
            ...
            metodo_nom = st.selectbox("Método de pago", ["efectivo", "transferencia"], key="nom_metodo")
            obs_nom = st.text_area("Observación (opcional)", key="nom_obs")

        if sueldo_bruto > 0:
            if arl_tasa_empleado is None:
                st.error(
                    "Configure la tasa ARL confirmada en el registro del empleado "
                    "antes de calcular o pagar la nómina."
                )
                return
            d = calcular_nomina_completa(
                sueldo_bruto,
                period_nom,
                arl_tasa=float(arl_tasa_empleado),
            )
            ...
            col_b3.metric(
                f"ARL ({float(arl_tasa_empleado)*100:.2f}%)",
                f"RD$ {d['arl_empleador']:,.2f}",
            )
            col_b4.metric("INFOTEP (1.00%)", f"RD$ {d['infotep_empleador']:,.2f}")
            col_b5.metric("💰 Costo del período", f"RD$ {d['costo_total_periodo']:,.2f}",
                           delta=f"+RD$ {d['tss_empleador']:,.2f} TSS empleador",
                           delta_color="inverse")

            st.info(f"📋 **Base ISR:** RD$ {d['base_isr']:,.2f} (sueldo mensual menos TSS empleado). "
                    f"El empleador asume un **{(d['tss_empleador']/d['sueldo_bruto']*100):.1f}% adicional** sobre el bruto del período. "
                    "La tasa ARL debe ajustarse al riesgo aprobado para la empresa.")
            ...
            with col_btn1:
                if st.button("✅ Registrar Pago de Nómina", key="btn_nom_reg", type="primary", use_container_width=True):
                    try:
                        from api_client import registrar_nomina

                        empleado_match = empleados_df[
                            empleados_df["nombre"].astype(str) == str(emp_sel)
                        ]
                        if empleado_match.empty or not empleado_match.iloc[0].get("id"):
                            raise ValueError("El empleado no tiene un identificador válido.")
                        result = registrar_nomina(
                            empleado_id=empleado_match.iloc[0]["id"],
                            periodo=period_nom,
                        ...
                        limpiar_cache_datos()
                        st.success(
                            f"✅ Nómina registrada de forma transaccional. "
                            f"Neto pagado: **RD$ {float(result.get('neto', 0)):,.2f}**"
                        )
                        st.rerun()
