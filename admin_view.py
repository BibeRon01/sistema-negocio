...
def render_usuarios():
    ...
    if not es_admin():
        ...
        usuarios_tab_labels = [
            "👥 Lista de Usuarios",
            "➕ Crear Usuario",
            "✏️ Editar / Eliminar Usuario",
        ]
        tab_destino = st.session_state.pop("_usuarios_tab_destino", "")
        if tab_destino in usuarios_tab_labels:
            # Se establece antes de construir el widget. Así, un rerun causado
            # por desactivar o eliminar no devuelve al usuario a la primera pestaña.
            st.session_state["usuarios_tabs"] = tab_destino
        ...
        if st.session_state.pop("_reset_user_editor_selection", False):
            st.session_state.pop("select_user_to_edit", None)
            st.session_state.pop("prev_selected_user_id", None)
                    ...
                    if st.button("💾 Guardar Cambios de Usuario", key="btn_save_user_changes", use_container_width=True):
                        try:
                            gestionar_usuario_seguro(
                                ...
                                rol=edit_rol,
                                activo=edit_activo,
                                permisos=permisos_editar,
                                nueva_password=edit_pass.strip(),
                            )
                            invalidar_cache_tabla("usuarios")
                            st.session_state["_reset_user_editor_widgets"] = True
                            st.session_state["_usuarios_tab_destino"] = usuarios_tab_labels[2]
                ...
                with c_btn2:
                    if st.button("⛔ Desactivar Usuario", key="btn_delete_user_changes", use_container_width=True):
                        if usr_sel["usuario"] == nombre_usuario_actual():
                            st.error("⚠️ No puede desactivar su propia sesión.")
                        else:
                            try:
                                gestionar_usuario_seguro(
                                    profile_id=usr_sel["id"],
                                ...
                                st.session_state["_usuarios_notice"] = (
                                    ...
                                    "Ya puede comprobar si admite eliminación definitiva."
                                )
                                st.rerun()
                            except ApiError as exc:
                                st.error(str(exc))
                ...
                if bool(usr_sel.get("activo", True)):
                    ...
                    if st.button(
                        ...
                        try:
                            ...
                            invalidar_cache_tabla("usuarios")
                            st.session_state["_reset_user_editor_widgets"] = True
                            st.session_state["_reset_user_editor_selection"] = True
                            st.session_state["_usuarios_tab_destino"] = usuarios_tab_labels[2]
                            st.session_state["_usuarios_notice"] = (
...
def render_configuracion():
    ...
    if not es_admin() and not tiene_permiso("puede_configurar"):
        ...
        if not cfg:
            ...
            st.markdown("---")
            st.subheader("🏛️ Información sobre secuencias DGII")
            st.warning(
                "La gestión de NCF está deshabilitada. Las secuencias y documentos "
                "fiscales se administran fuera de A&M según el procedimiento oficial."
            )
            
            st.info(
                "A&M no consulta ni modifica secuencias NCF operativas. Confirme "
                "autorizaciones, rangos y vencimientos directamente mediante el "
                "procedimiento oficial que corresponda."
            )

            with st.expander("Información de campos — solo lectura", expanded=False):
                st.markdown(
                    """
                    Las autorizaciones fiscales pueden incluir un tipo de
                    comprobante, rango o secuencia y una vigencia. Esos datos no
                    se capturan aquí. El propietario debe conservar la
                    autorización correspondiente y confirmar su uso con la DGII
                    o con el proveedor fiscal autorizado.
                    """
                )
                ...
                if col_btn[0].button("💾 Guardar Permisos y Cambios", key="btn_save_emp_permissions", use_container_width=True):
                    try:
                        gestionar_usuario_seguro(
                            ...
                            rol=e_rol,
                            activo=e_activo,
                            permisos=permisos_emp,
                            nueva_password=e_clave.strip(),
                        )
                ...
                if col_btn[1].button("⛔ Desactivar Cuenta", key="btn_delete_emp_account", use_container_width=True):
                    try:
                        gestionar_usuario_seguro(
                            ...
                            activo=False,
                            permisos=dict(selected_user.get("permissions") or {}),
                        )
                        st.success(f"Cuenta de {selected_user['usuario']} desactivada; el historial se conserva.")
                        limpiar_cache_datos()
                        st.rerun()
                    except ApiError as exc:
                        st.error(str(exc))
            ...
            with st.expander("✨ Haz clic para crear una nueva cuenta de empleado", expanded=False):
                ...
                if st.button("🚀 Crear Cuenta de Empleado", key="btn_create_new_employee", use_container_width=True):
                    ...
                    if not usuario_nuevo or not n_nombre or not n_clave:
                        ...
                        try:
                            permisos_nuevo = {
                                key: value for key, value in new_user_payload.items()
...
def render_mi_perfil():
    ...
    with tab_perfil:
        ...
        if not user_id or str(user_id).strip() == "None":
            st.error(
                "El perfil no está vinculado correctamente con Supabase Auth. "
                "Cierre sesión y solicite al administrador que repare la cuenta."
            )
            st.stop()
            ...
            if st.form_submit_button("💾 Guardar cambios", type="primary", use_container_width=True):
                ...
                if not name_clean:
                    ...
                    try:
                        ...
                        if clave_clean:
                            supabase.auth.update_user({"password": clave_clean})
                        user["nombre"] = name_clean
    ...
    with tab_plan:
        ...
        u4.metric("💰 Ventas internas del mes", f"RD$ {total_ventas_mes:,.2f}")
        ...
        MODULOS_SISTEMA = [
            ...
            ("💳", "Créditos y CxC", True),
            ("📋", "Orientación sobre notas de crédito", True),
            ("🏛️", "Orientación sobre comprobantes gubernamentales", True),
            ("📑", "Nómina con parámetros versionados", True),
            ("📊", "Academia sobre 606/607/IT-1/IR-2", True),
            ("📒", "Libro Mayor", True),
