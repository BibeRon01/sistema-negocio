# SQL de instalación y reparación de A&M

Esta es la guía completa. Ejecute primero en **staging** y conserve un respaldo verificado. En Supabase SQL Editor pegue únicamente el contenido que está dentro de cada bloque `sql`; no pegue títulos que empiecen con `#` ni las tres comillas invertidas.

Para corregir ahora Caja y Cobrar en la base ya existente, no repita toda la instalación: ejecute directamente el archivo `supabase/migrations/202609050002_pos_cash_open_account_repair.sql` y luego `supabase/checks/004_pos_cash_repair_readonly.sql`.

Cada migración transaccional termina con `commit;`. Si aparece un error antes de ese punto, PostgreSQL revierte ese bloque completo.
...
## 1. Base segura, Auth, empresas, RLS y tablas
...
## 2. Ventas, caja, créditos e inventario transaccionales
...
## 3. Mantenimiento, cuentas abiertas, compras, nómina y contabilidad
...
```sql
-- A&M: compatibilidad no destructiva de auditoría requerida por Caja.
-- Este archivo puede ejecutarse completo en Supabase SQL Editor.
...
where table_schema='public'
  ...
  and column_name in (
      'empresa_id','usuario_id','usuario','accion','modulo','tabla',
      'registro_id','detalle','metadata','created_at','evento','detalles'
  )
...
```sql
-- A&M: corrige la apertura de caja cuando dia_operativo es DATE.
-- No cambia ni elimina cajas existentes.
...
## 10. Reparación comprobada de Caja, Cobrar y cuentas abiertas

Este es el bloque nuevo que corresponde al diagnóstico real del 5 de septiembre de 2026. No elimina historial. Convierte a UUID únicamente campos de identificación validados y limpia solo membresías cuyo usuario ya no existe en Supabase Auth.
...
## 11. Verificación específica de la reparación

Los resultados de columnas y funciones deben aparecer. `membresias_sin_auth` debe ser 0, la consulta de cajas duplicadas debe estar vacía y las cuatro comprobaciones booleanas del final deben decir `true`. La consulta de productos negativos puede mostrar el producto que requiere conteo físico; no lo corrija automáticamente.
...
## 12. Verificación general posterior
