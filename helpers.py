...
# =========================================================
# CÁLCULO INTERNO DE ITBIS (NO GENERA DOCUMENTOS DGII)
# =========================================================
...
PLANES_AM = {
    ...
    "premium": {
        ...
        "modulos": ["Dashboard", "Caja", "Dinero Real", "POS", "Ventas", "Clientes", "Productos",
                    ...
                    "Estado de Resultados", "Informes", "Academia DGII", "Créditos",
                    "Activos Fijos", "Capital Base"],
        "max_usuarios": 8,
        "descripcion": "Suite financiera con nómina y academia educativa DGII.",
    },
    "enterprise": {
        "nombre": "Enterprise",
        "precio_mensual": 4500,
...
def registrar_compra_producto(producto_row: pd.Series, cantidad: float, costo_unitario: float, fecha_compra: str, proveedor: str = "", numero: str = "", descripcion: str = "", metodo: str = "") -> bool:
    ...
    try:
        from api_client import registrar_compra_producto as registrar_compra_api

        registrar_compra_api({
...
def _leer_tabla_de_supabase(nombre_tabla: str, order_by: str = "id", tenant: str = "global") -> pd.DataFrame:
    """Descarga la tabla; RLS es la autoridad y el filtro local es defensa adicional."""
    try:
...
def eliminar_venta_completa_app(venta_id: Any) -> bool:
    st.error(
        "Las ventas no se eliminan porque forman parte del historial contable. "
        "Use la anulación con un motivo documentado."
    )
    return False
...
def anular_venta_completa_app(venta_id: Any, motivo: str = "") -> bool:
    try:
        from api_client import anular_venta

        anular_venta(venta_id, motivo)
        for tabla in [
            "ventas", "detalle_venta", "ventas_pagos", "movimientos_caja",
            "cuentas_por_cobrar", "inventario_lotes", "productos",
...
def crear_cliente_rapido_pos(nombre, telefono="", documento="", direccion="", email=""):
    ...
    for payload in intentos:
        try:
            payload["empresa_id"] = obtener_tenant_actual()
...
def abrir_caja(monto_inicial: float, observacion: str = "") -> tuple[bool, str]:
    ...
    try:
        ...
        abrir_caja_api(float(monto_inicial), observacion)
        limpiar_cache_datos()
...
def cerrar_caja(caja_row: dict, monto_cierre: float, observacion: str = "") -> tuple[bool, str]:
    try:
        from api_client import cerrar_caja as cerrar_caja_api

        cerrar_caja_api(caja_row["id"], float(monto_cierre), observacion)
        limpiar_cache_datos()
...
def construir_html_impresion(post_venta: dict, tipo: str = "factura") -> str:
    ...
    referencia_interna = html_escape(
        post_venta.get("numero_factura") or post_venta.get("venta_id") or ""
    )
    ...
    <body>
      <div class="wrap">
        <div class="top">
          ...
          <p><strong>Referencia interna:</strong> {referencia_interna}</p>
          <p><strong>Sin valor fiscal. No es NCF ni e-CF.</strong></p>
...
def registrar_abono_credito_seguro(fila, monto, metodo_pago, observacion=""):
    ...
    try:
        from api_client import registrar_abono

        caja_activa = obtener_caja_abierta()
        if caja_activa is None:
            st.error("Debe tener una caja abierta para registrar el abono.")
            return False
        result = registrar_abono(
            ...
            metodo_pago=metodo_pago,
            caja_id=caja_activa.get("id"),
            observacion=observacion,
        )
        for tabla in ["cuentas_por_cobrar", "abonos_credito", "movimientos_caja", "movimientos_contables"]:
            invalidar_cache_tabla(tabla)
        st.success(
            f"Abono registrado de forma transaccional: "
            f"RD$ {float(result.get('monto_aplicado', monto)):,.2f}."
        )
        return True
    except Exception as exc:
...
# =========================================================
# ACCESO SEGURO V3 — SUPABASE AUTH + SESIÓN SQL + MFA NATIVO
# Esta definición sustituye el acceso heredado situado más arriba.
# =========================================================
def _auth_obj_value(obj, key, default=None):
    if obj is None:
        return default
...
def _guardar_tokens_auth(session) -> None:
    if session is None:
        raise RuntimeError("Supabase no devolvió una sesión válida.")
    token_source = _auth_obj_value(session, "session", None) or session
    access_token = str(_auth_obj_value(token_source, "access_token", "") or "")
    refresh_token = str(_auth_obj_value(token_source, "refresh_token", "") or "")
    if not access_token or not refresh_token:
        raise RuntimeError("Supabase no devolvió tokens de sesión completos.")
    st.session_state["access_token"] = access_token
    st.session_state["refresh_token"] = refresh_token
    from db import renovar_cliente_sesion
    renovar_cliente_sesion()


def _query_param_unico(nombre: str) -> str:
    """Obtiene un parámetro de URL sin aceptar colecciones ambiguas."""
    try:
        value = st.query_params.get(nombre, "")
    except Exception:
        return ""
    if isinstance(value, (list, tuple)):
        return str(value[0] if len(value) == 1 else "").strip()
    return str(value or "").strip()


def _render_recuperacion_password() -> bool:
    """Verifica un enlace recovery de Supabase y permite fijar una clave nueva.

    El enlace debe transportar ``token_hash``; nunca se colocan access tokens ni
    contraseñas en la URL. La sesión temporal también la emite Supabase Auth.
    """
    token_hash = _query_param_unico("token_hash")
    recovery_type = _query_param_unico("type").lower()
    recovery_active = bool(st.session_state.get("password_recovery_active"))

    if token_hash and recovery_type == "recovery" and not recovery_active:
        try:
            response = supabase.auth.verify_otp({
                "token_hash": token_hash,
                "type": "recovery",
            })
            _guardar_tokens_auth(response)
            st.session_state["password_recovery_active"] = True
            st.session_state["password_recovery_verified"] = True
            st.query_params.clear()
            recovery_active = True
        except Exception as exc:
            LOGGER.warning("Enlace de recuperación rechazado: %s", type(exc).__name__)
            limpiar_estado_sesion(cerrar_auth=True)
            try:
                st.query_params.clear()
            except Exception:
                pass
            st.error("El enlace de recuperación no es válido o ya venció. Solicite uno nuevo.")
            return False

    if not recovery_active:
        return False

    st.markdown("### Crear una contraseña nueva")
    st.caption("El enlace fue verificado por Supabase. La contraseña no se guarda en la aplicación.")
    nueva = st.text_input(
        "Nueva contraseña",
        type="password",
    ...
    confirmar = st.text_input(
        ...
        type="password",
        key="secure_recovery_password_confirm",
    )
    ...
    if left.button("Guardar contraseña", type="primary", use_container_width=True):
        ...
        try:
            ...
            if client is None or not st.session_state.get("password_recovery_verified"):
                raise RuntimeError("RECOVERY_SESSION_REQUIRED")
            client.auth.update_user({"password": nueva_clean})
            limpiar_estado_sesion(cerrar_auth=True)
            st.success("Contraseña actualizada. Ya puede iniciar sesión.")
        except Exception as exc:
            LOGGER.warning("No se pudo actualizar la contraseña: %s", type(exc).__name__)
            limpiar_estado_sesion(cerrar_auth=True)
            st.error("No se pudo actualizar la contraseña. Solicite un enlace nuevo.")
        return True
...
def _render_mfa_nativo() -> bool:
    ...
    st.markdown("### 🔐 Segundo factor obligatorio")
    st.caption("Los administradores deben confirmar un código de su aplicación autenticadora.")
...
def login_simple() -> bool:
    ...
    st.markdown(
        """
        <div style="max-width:520px;margin:3rem auto 1rem;text-align:center">
          <h1 style="margin-bottom:.25rem">Sistema Contable A&amp;M</h1>
          <p style="color:#6b7280">Acceso protegido para su negocio y punto de venta</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if st.button("Entrar", type="primary", use_container_width=True, key="secure_login_submit"):
        ...
        auth_success = False
        authenticated_by_supabase = False
        ...
        if not auth_success:
            limpiar_estado_sesion(cerrar_auth=True)
            if authenticated_by_supabase:
                st.error(
                    "Supabase validó la cuenta, pero todavía no tiene el acceso A&M o "
                    "la membresía de empresa requeridos."
                )
            else:
                st.error("Usuario o correo y contraseña incorrectos.")

    with st.expander("¿Olvidó su contraseña?", expanded=False):
        reset_email = st.text_input(
            "Correo A&M registrado",
            value=str(identifier_input or "") if "@" in str(identifier_input or "") else "",
            key="secure_reset_email",
        )
        if st.button("Enviar enlace de recuperación", key="secure_reset_submit"):
            ...
            if value.count("@") != 1 or value.startswith("@") or value.endswith("@"):
                ...
                try:
                    ...
                    LOGGER.warning("Solicitud de recuperación rechazada: %s", type(exc).__name__)
                st.info("Si la cuenta existe, Supabase enviará un enlace de recuperación.")
        st.caption(
            "Los usuarios empresariales sin correo solicitan una contraseña nueva a su administrador."
        )
