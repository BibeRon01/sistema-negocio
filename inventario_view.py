...
def render_compras():
        ...
        if not df_hist_c.empty:
            ...
            if df_hist_c.empty:
                ...
                if puede_editar_compras() or puede_eliminar_compras():
                    st.info(
                        "Las compras registradas ya no se editan ni eliminan directamente. "
                        "Una corrección debe hacerse con una anulación transaccional para "
                        "conservar inventario, contabilidad y auditoría."
                    )
                if False and (puede_editar_compras() or puede_eliminar_compras()):
                    st.markdown("---")
    ...
    with tab_utilidad_historico:
        ...
        st.caption(
            "Consolidado calculado desde las ventas de esta empresa y el resumen "
            "histórico migrado; no depende de archivos privados guardados en una computadora."
        )
        df_fin = DATA.get("detalle_venta", pd.DataFrame()).copy()
        df_hist = DATA.get("ventas_producto_historico", pd.DataFrame()).copy()
        if not df_hist.empty:
            hist_normalizado = pd.DataFrame({
                "producto": df_hist.get("producto", "Producto sin nombre"),
                "cantidad": pd.to_numeric(df_hist.get("cantidad", 0), errors="coerce").fillna(0),
                "costo_unitario": pd.to_numeric(df_hist.get("costo_unitario", 0), errors="coerce").fillna(0),
                "total_linea": pd.to_numeric(df_hist.get("total_vendido", 0), errors="coerce").fillna(0),
                "ganancia_linea": pd.to_numeric(df_hist.get("utilidad", 0), errors="coerce").fillna(0),
                "anulado": False,
            })
            df_fin = pd.concat([df_fin, hist_normalizado], ignore_index=True, sort=False)
        if df_fin.empty:
            st.info("Todavía no existen detalles de ventas para calcular este reporte.")
        else:
            if "anulado" in df_fin.columns:
                df_fin = df_fin[~df_fin["anulado"].fillna(False).astype(bool)]

            producto_col = "producto" if "producto" in df_fin.columns else "nombre"
            if producto_col not in df_fin.columns:
                df_fin[producto_col] = "Producto sin nombre"

            def numero_columna(nombre, valor=0.0):
                if nombre not in df_fin.columns:
                    return pd.Series(valor, index=df_fin.index, dtype="float64")
                return pd.to_numeric(df_fin[nombre], errors="coerce").fillna(valor)

            cantidades = numero_columna("cantidad")
            costos = cantidades * numero_columna("costo_unitario")
            ventas_calculadas = cantidades * numero_columna("precio_unitario")
            ventas = (
                numero_columna("total_linea")
                if "total_linea" in df_fin.columns
                else ventas_calculadas
            )
            utilidad = (
                numero_columna("ganancia_linea")
                if "ganancia_linea" in df_fin.columns
                else ventas - costos
            )

            base = pd.DataFrame({
                "Producto": df_fin[producto_col].fillna("Producto sin nombre").astype(str),
            ...
            resumen = (
                base.groupby("Producto", as_index=False)
                .sum(numeric_only=True)
                .sort_values("Utilidad", ascending=False)
            )

            txt_fin = st.text_input(
                "🔍 Buscar producto en el reporte de utilidad",
                key="buscar_utilidad_fin",
            )
            if txt_fin:
                resumen = resumen[
                    resumen["Producto"].str.contains(txt_fin, case=False, na=False)
                ]

            col1, col2, col3 = st.columns(3)
            col1.metric("Costo Total Acumulado", f"RD$ {resumen['Total Costo'].sum():,.2f}")
            col2.metric("Venta Total Acumulada", f"RD$ {resumen['Total Vendido'].sum():,.2f}")
            col3.metric("Utilidad Acumulada", f"RD$ {resumen['Utilidad'].sum():,.2f}")
            st.dataframe(resumen, use_container_width=True)
            descargar_archivos(resumen, "utilidad_historica_por_producto")
