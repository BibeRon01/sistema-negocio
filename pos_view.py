...
def render_pos():
    ...
    if productos_df.empty:
        ...
        def mostrar_cuentas_abiertas_activas():
            ...
            if ventas_all.empty or "estado" not in ventas_all.columns:
                st.info("No hay cuentas abiertas registradas.")
                return
            cuentas_seguras = ventas_all[
                (ventas_all["estado"].astype(str).str.lower() == "abierta")
                & (~ventas_all.get("anulado", False).fillna(False).astype(bool))
            ]
            if cuentas_seguras.empty:
                st.info("No hay cuentas abiertas activas en este momento.")
                return

            st.caption(
                "Las cuentas se editan o cancelan mediante operaciones atómicas. "
                "Dividir, fusionar y cobrar participantes permanecen desactivados "
                "hasta contar con su flujo transaccional propio."
            )
            for _, cuenta in cuentas_seguras.iterrows():
                cuenta_id = cuenta.get("id")
                alias = cuenta.get("cliente_nombre") or "Sin nombre"
                with st.container(border=True):
                    info_col, edit_col, void_col = st.columns([3, 1, 2])
                    info_col.markdown(
                        f"**{alias}** · {cuenta.get('numero_factura') or 'Recibo interno'}  \n"
                        f"RD$ {float(cuenta.get('total') or 0):,.2f}"
                    )
                    if edit_col.button(
                        "✏️ Editar",
                        key=f"safe_open_edit_{cuenta_id}",
                        use_container_width=True,
                    ):
                        detalles = (
                            supabase.table("detalle_venta")
                            .select("*")
                            .eq("venta_id", str(cuenta_id))
                            .execute()
                            .data
                            or []
                        )
                        st.session_state["pos_carrito"] = [
                            {
                                ...
                                "nombre": item.get("producto") or "Producto",
                        ...
                        st.session_state["pos_cuenta_abierta_id"] = cuenta_id
                        st.session_state["pos_cuenta_abierta_nombre"] = alias
                        st.session_state["_pos_ir_a_carrito"] = True
                        st.rerun()

                    motivo = void_col.text_input(
                        "Motivo de cancelación",
                        key=f"safe_open_reason_{cuenta_id}",
                        placeholder="Mínimo 10 caracteres",
                    )
                    if void_col.button(
                        "🚫 Cancelar",
                        key=f"safe_open_void_{cuenta_id}",
                        use_container_width=True,
                    ):
                        if len(motivo.strip()) < 10:
                            st.error("Escribe un motivo de al menos 10 caracteres.")
                        else:
                            try:
                                from api_client import anular_venta

                                anular_venta(cuenta_id, motivo)
                                limpiar_cache_datos()
        ...
        if carrito:
            ...
            # El POS emite únicamente comprobantes internos. Los documentos DGII
            # se gestionan fuera de esta aplicación siguiendo el proceso oficial.
            st.markdown("### 🧾 Documento interno de venta")
            st.info("Este recibo no tiene valor fiscal y no genera NCF ni e-CF.")
            fact_fiscal_on = False
            ...
            if es_cuenta_editada:
                ...
                try:
                    ...
                    if False and v_resp.data:  # Flujo de participantes desactivado hasta tener RPC atómico.
                        v_row = v_resp.data[0]
            ...
            if proceder:
                ...
                if True:
                    try:
                            ...
                            if es_cuenta_editada:
                                from api_client import reemplazar_cuenta_abierta

                                rpc_res = reemplazar_cuenta_abierta(
                                    st.session_state["pos_cuenta_abierta_id"],
                                    payload_unificado,
                                )
                            else:
                            ...
                            factura_final = rpc_res.get("numero_factura") or numero_factura_pos
                            total_confirmado = float(rpc_res.get("total", total_real_venta))
                            subtotal_confirmado = float(rpc_res.get("subtotal", subtotal_neto))
                            itbis_confirmado = float(rpc_res.get("itbis_total", total_itbis_neto))
                            ...
                            if es_cobro:
                                st.session_state["pos_post_venta"] = {
                                    ...
                                    "total_real": total_confirmado,
                                    "subtotal": subtotal_confirmado,
                                    "itbis_total": itbis_confirmado,
...
def render_ventas():
    ...
    if False and puede_editar_ventas():
    ...
    if not df.empty:
        ...
        if puede_gestionar_ventas:
            st.subheader("✏️ Editar cuenta abierta")

            ventas_visibles = df.copy()
            if "estado" in ventas_visibles.columns:
                ventas_visibles = ventas_visibles[
                    ventas_visibles["estado"].astype(str).str.lower() == "abierta"
                ]
            if ventas_visibles.empty:
                st.info(
                    "No hay cuentas abiertas editables. Una venta completada se "
                    "conserva; si necesita corregirla, debe anularla con un motivo "
                    "y registrar una operación nueva."
                )
            if not ventas_visibles.empty:
                ...
                if has_ncf:
                    st.warning(
                        "La referencia NCF histórica es inmutable. Registre una "
                        "anulación interna documentada y gestione la corrección "
                        "fiscal fuera de A&M."
                    )
                ...
                if detalle_df.empty and not has_ncf:
                    ...
                    if st.button("💾 Guardar edición completa", key="btn_guardar_edicion_completa"):
                        try:
                            from api_client import editar_venta
                            ...
                            result = editar_venta(venta_id, items_api, metodo_pago_nuevo)
                            limpiar_cache_datos()
                            st.success(
                                "Cuenta abierta actualizada de forma segura. "
                                f"Nueva referencia: {result.get('numero_factura', result.get('venta_id'))}"
                            )
                            st.rerun()
                        except Exception as exc:
            ...
            with st.expander("🛠️ Control rápido / anular ventas", expanded=False):
                ...
                if opciones:
                    ...
                    with cl1:
                        if has_ncf:
                            st.error(
                                "Referencia NCF histórica: edición bloqueada. "
                                "La corrección fiscal se realiza fuera de A&M."
                            )
                        elif False and puede_editar_ventas() and st.button("💾 Guardar datos generales", key="btn_guardar_cambios_venta"):
                    ...
                    with cl3:
                        ...
                        elif False and puede_eliminar_ventas() and st.button("🗑️ Eliminar venta", key="btn_eliminar_venta_admin"):
...
def render_caja():
    ...
    def _cerrar_caja(caja, efectivo_contado, obs_cierre, usuario_cierre=None):
        try:
            from api_client import cerrar_caja

            cerrar_caja(caja.get("id"), float(efectivo_contado), obs_cierre)
            limpiar_cache_datos()
            return True
        except Exception as exc:
    ...
    if es_admin():
        ...
        if cajas_admin.empty:
            ...
            st.info(
                "Las cajas cerradas son inmutables. Para corregir una diferencia "
                "se debe crear un ajuste transaccional y auditado; no se modifica "
                "el cierre original."
            )
            with st.expander("🔄 Recalcular caja cerrada", expanded=False):
                ...
                if cajas_cerradas_recalc.empty:
                    ...
                    if False and st.button("Aplicar recálculo a esta caja", key="admin_aplicar_recalculo_caja"):
                        payload_recalc = {
            ...
            with st.expander("✏️ Editar datos de una caja cerrada", expanded=False):
                ...
                if cajas_cerradas.empty:
                    ...
                    if False and st.button("Guardar corrección de caja", key="admin_guardar_correccion_caja"):
                        payload_edit = {
...
def render_creditos():
    ...
    def registrar_abono_general_fifo(cliente_nombre: str, monto_total: float, metodo_pago: str, observacion: str = "") -> bool:
        try:
            ...
            if not clientes.empty and "nombre" in clientes.columns:
                match = clientes[
                    clientes["nombre"].astype(str).apply(normalizar_texto)
                    == normalizar_texto(cliente_nombre)
                ]
                if not match.empty:
                    cliente_id_api = match.iloc[0].get("id")
            result = registrar_abono(
                cliente_id=cliente_id_api,
            ...
            limpiar_cache_datos()
            st.success(
                f"Abono aplicado de forma transaccional: "
                f"RD$ {float(result.get('monto_aplicado', monto_total)):,.2f}."
            )
            return True
        except Exception as exc:
    ...
    puede_abonar_credito = es_admin() or tiene_permiso("ver_credito")
    ...
    if cxc.empty:
        ...
        with tab_expediente:
            ...
            if not clientes_disponibles:
                ...
                # Vista de corrección
                if False and puede_editar_credito:  # Correcciones directas desactivadas; usar abonos auditados.
                    st.markdown("---")
